...
- Выбирает приз
- Списывает активности (content/code) или 2 кода (в зависимости от сценария)
- Отдаёт награду
...
3. Добавляются призы уровня **silver** в количестве:
    - `spin.pool.prizes.number - (места, занятые балансами из п.2)`
...
4. Добавляются призы уровня **gold** в количестве:
    - `spin.pool.prizes.number - (места, занятые в п.2 и п.3)`
...
- Вероятность выпадения **баланса** на уровне L определяется:
    ...
    - и ограничением: **балансов не может быть больше, чем `spin.pool.prizes.number`**
